...
public class ApplicationFormAutoLoanDao  extends GenericDao<Integer, ApplicationFormAutoLoan>{

	private static final long serialVersionUID = -3491793157114251423L;
	...
	public ApplicationFormAutoLoan getApplicationFormAutoLoanByAppReferenceIdAndMobileNo(String appReferenceId, String appMobileNo) throws SQLException{
        ...
        List<ApplicationFormAutoLoan> list = findByNamedQuery("ApplicationFormAutoLoan.getApplicationFormAutoLoanByAppReferenceIdAndMobileNo", params);
        if(list.size()==1){
        	return list.get(0);
        }
        return null;
	...
	public ApplicationFormAutoLoan getApplicationFormAutoLoanByQuoteId(Integer  appQuoteId) throws SQLException, NoResultException{
        Map<String, Object> params = new HashMap<String, Object>(1);
        params.put("appQuoteId", appQuoteId);        
        List<ApplicationFormAutoLoan> list = findByNamedQuery("ApplicationFormAutoLoan.getApplicationFormAutoLoanByQuoteId", params);
        if(list.size()==1){
        	return list.get(0);
        }
        return null;
	...
	public ApplicationFormAutoLoan getApplicationFormAutoLoanByAppReferenceId(String appReferenceId) throws SQLException, NoResultException{
        Map<String, Object> params = new HashMap<String, Object>(1);
        params.put("appReferenceId", appReferenceId);        
        List<ApplicationFormAutoLoan> list = findByNamedQuery("ApplicationFormAutoLoan.getApplicationFormAutoLoanByAppReferenceId", params);
        if(list.size()==1){
        	return list.get(0);
        }
        return null;
	...
	public String getLastGeneratedReferenceNumber(Integer loanTypeId) throws SQLException{
		...
		String query = "SELECT MAX(APP_REFERENCE_ID) FROM RUPEEPOWER_OCAS_T_00063 WHERE APP_REFERENCE_ID like '"+Constants.INTIAL_STRING_AL+Constants.SOURCE_STRING_AL+currentMonth+currentYear+"%'";
	...
	public boolean isAppFoundForDedupInDropRejectStage(String isdCode, String appMobileNo, Integer loanPurposeId) throws SQLException{
		...
		String query = "SELECT  COUNT(APP_SEQ_ID)  FROM RUPEEPOWER_OCAS_T_00063 A, RUPEEPOWER_OCAS_T_02024 S "
		...
		if(!"0".equalsIgnoreCase(Constants.APP_DUPLICATION_TIME_PERIOD.toString())){
			...
			logger.info("APP_ENTRY_TIME : "+APP_ENTRY_TIME);
	...
	public boolean isReferenceIdAvailable(String appRefererenceId) throws SQLException{
	    ...
	    String nativeQuery="SELECT APP_REFERENCE_ID FROM RUPEEPOWER_OCAS_T_00063 WHERE APP_REFERENCE_ID = ?";
	...
	public Integer getVisitByAppSeqIdAndEntityName(Integer appSeqId, String visitTableName){
	    ...
	    nativeQuery +=" FROM RUPEEPOWER_OCAS_T_00063 APP, RUPEEPOWER_OCAS_T_00099 QUOTE, "+visitTableName+" VISIT ";
	    nativeQuery +=" WHERE ";
	...
	public ApplicationFormAutoLoan getApplicationFormAutoLoanByMobileAndSmsOtp(String appMobileNo,Integer appMobileVerificationCode) throws SQLException, NoResultException{ 
		Map<String, Object> params = new HashMap<String, Object>(2);
		 params.put("appMobileVerificationCode", appMobileVerificationCode);
	     params.put("appMobileNo", appMobileNo);
        List<ApplicationFormAutoLoan> list = findByNamedQuery("ApplicationFormAutoLoan.getApplicationFormAutoLoanByMobileAndSmsOtp",params);
        if(list.size()>0){
        	return list.get(0);
        }
        return null;
