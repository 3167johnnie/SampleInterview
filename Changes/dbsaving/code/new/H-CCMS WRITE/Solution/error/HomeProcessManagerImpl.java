    ...
    try {
      ...
      if (stateId.intValue() == 3 || stateId.intValue() == 4 || stateId.intValue() == 14 || stateId.intValue() == 15) {
        ...
        if (appApplyingFrom == 2) {
         ...
         // logger.info("mobile check1 while send OTP::" + mobile);
          ...
          logger.info("isAlternate check2 while send OTP::" + isAlternate);
      ...
      //  logger.info("inputOtp is Not Alternate : " + inputOtp);
       // logger.info("appOTPVerifyType is Not Alternate : " + appOTPVerifyType);
        logger.info("stateId is Not Alternate : " + stateId);
        ...
        if (isOPTVerified) {
          ...
          logger.info("HomeProcessManagerImpl.java LNO ::900 , log for CALLBACK_SMS_CONSENT " + Constants.CALLBACK_SMS_CONSENT);
          if (Constants.CALLBACK_SMS_CONSENT) {
            logger.info("HomeProcessManagerImpl.java LNO ::902 , log for CALLBACK_SMS_CONSENT " + application.getAppMobileVerified());
            if ("Y".equalsIgnoreCase(application.getAppMobileVerified())) {
              logger.info("HomeProcessManagerImpl.java LNO ::903 , log for CALLBACK_SMS_CONSENT " + Constants.MESSAGE_TYPE_SMS);
          ...
          if (stateId.intValue() == 5) {
            logger.info("ProcessManagerImpl LNO ::926 :: message has been sent to user ");
       //     logger.info("cheking orginal moblie number io if    1244 from Session "+SessionUtil.getMobile());
            json.put("message", "Thank you for your interest. Our representative will contact you shortly.");
          } else {
      //  	  logger.info("cheking orginal moblie number io else   1246 from Session "+SessionUtil.getMobile());
        //    logger.info("OTP verfied for mobileNo ::" + application.getAppMobileNo());
            json.put("message", "OTP authentication successful");
          } 
          json.put("alertCount", alertCount);
          return json;
        } 
        //logger.info("OTP is incorrect for mobileNo ::" + application.getAppMobileNo());
        json.put("status", "error");
...
//		Integer consentId = (consent!= null && consent.getConsentId() != null) ? consent.getConsentId() : null;
        ...
        //logger.info("Home lOan data 9 " + quote.toString());
        quote = this.homeLoanService.save(quote);
        //logger.info("HomeLoanProcessImpl.java LNo: 2468 :: " + quote);
        if (quote == null) {
        ...
        //logger.info("save quote object :: " + quote.toString() + " with AppSeqId " + appSeqId);
      } catch (ParseException ex) {
