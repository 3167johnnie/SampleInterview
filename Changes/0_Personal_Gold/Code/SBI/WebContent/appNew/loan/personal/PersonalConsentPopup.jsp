...
<div id="termAndConditionHTML">
	<div class="modal fade otp-box" id="consentPersonalLoanNtb" tabindex="-1"
		...
		<div class="modal-dialog" role="document">
			<div class="modal-content">
				<div class="privacy-modal-body">
					...
					<%-- <div id="consentPersonalLoanNtbDiv" class="f-otp-pop-content ">
						<s:property escapeHtml="false" value="%{beanList.consentPersonalLoanNtb}" />
						
					</div> --%>
					<button type="button" class="close" data-bs-dismiss="modal" aria-label="Close" style="margin-right: -60px; margin-top: -20px;">
...
                    </button>
					...
					<div id="consentPersonalLoanNtbDiv" class="privacy-consent-pop-content">Loading
						Privacy Notice...</div>
...
<script>
				...
				.ajax({
					...
					success : function(response) {
						...
						if (json.status == "success") {
							$("#consentPersonalLoanNtbDiv").html(json.privacyNotice);
							$("#consentPersonalLoanNtbDiv").scrollTop(0);
							resetConsentScrollValidation();
						} else {
							$("#consentPersonalLoanNtbDiv").html(
									"Privacy Notice Not Found");
						}
					},
					error : function() {
						$("#consentPersonalLoanNtbDiv").html0(
	...
	$(document).on("show.bs.modal", "#consentPersonalLoanNtb", function(e) {
		if (!canOpenPrivacyPopup()) {
			e.preventDefault();
			return false;
		}
	});
	$(document).on("shown.bs.modal", "#consentPersonalLoanNtb", function() {
	...
	$(document).on("scroll", "#consentPersonalLoanNtbDiv", function() {
		var div = $(this)[0];
	...
	function resetConsentScrollValidation() {
		...
		$("#consentPersonalLoanNtbDiv").off("scroll").on("scroll", function() {
			var div = $(this)[0];
			if (div.scrollTop + div.clientHeight >= div.scrollHeight - 5) {
				$("#acceptConsentBtn").prop("disabled", false).css({
					"opacity" : "1",
		...
		var div = $("#consentPersonalLoanNtbDiv")[0];
	...
	function acceptPrivacyConsent() {
		...
		$("#consentPersonalLoanNtb").modal("hide");
	}
	function loadPrivacyLanguageDropdown() {
		$
				.ajax({
					url : "getPrivacyLanguageListPersonal",
					type : "POST",
					success : function(response) {
