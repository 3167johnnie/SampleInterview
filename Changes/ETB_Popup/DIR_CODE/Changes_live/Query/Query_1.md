        ...
        jQuery(document).ready(function(){
                $('body').bind('cut copy paste',function(e){
                        e.preventDefault();
							...
							<s:include value="/appNew/common/ConsentPopupCve.jsp"></s:include>
				...
				<div id="termsAndConditionCBS" class="sbi-trms-div sbi-trms-div-cbs" style="display :<s:property value="%{showCBS?'block':'none'}"/>;">
					<ul class="form-section">
						<li class="full-width">
							...
							<div class="trms-section">
				<!-- 	<input type="checkbox" class="blue-css-checkbox" name="infoprovideCve" id="infoprovideCve" value="on" disabled="disabled">
					<label for="infoprovideCve" class="label-content" style="font-size:14px; ">
...
<div id="termsAndConditionCBS" class="sbi-trms-div sbi-trms-div-cbs" style="display :<s:property value="%{showCBS?'block':'none'}"/>;">
	...
	/* jQuery(document).ready(function(){
		...
		$('#infoprovideCve').change(function() {
		    if ($(this).is(':checked')) {
		    } else {
		    	$('#infoprovideCve').prop('disabled', true);
		    }
		});
	...
	jQuery(document).ready(function(){
		...
		above is the code give the code for this that submit should be done block if checkbox is not selected it shoud not proceed further it should say read the consent 
		<input type="checkbox" class="blue-css-checkbox" name="infoprovide" id="infoprovide" value="on" checked="checked" style="display:none">
		onces popup is openend and accepted it will check the checkbox this is working in above page 
		but when user doesnt open the popup and directly submit it getting submmitted 
		this is below submit button it should verfiy the checkbox is selected then it should submitted 
		<div class="qt-btn-div trms-btn flr mrgt-10">
								<input  type="submit" class="submit-btn" value="Submit" id="submitBtn">
							</div>
							
							
							else it not not allow submit further for submit button it should higlight the checkbox .......
							
							give minimnal code changes in above existing code line by line in details  
