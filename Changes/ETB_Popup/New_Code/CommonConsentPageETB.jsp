						...
						<s:if test="%{#containerTitle == 'Essential Purposes' || #containerTitle == 'Optional Purposes'}">
							<tr>
								...
								<s:if test="%{#containerTitle == 'Essential Purposes'}">
									<input type="hidden" name="essentialPurpose" value="<s:property value='code'/>|<s:property value='version'/>" />
									 <s:if test="%{#stat.first}">
        <td style="border: 1px solid rgb(140 140 140)" rowspan="<s:property value='rowSpanCount' />">
            <a href="javascript:void(0);" onclick="javascript:openEtbPrivacyPopup('consentPopupETB','1');" style="cursor:pointer;">
                <input type="checkbox" id="essentialPurpose" checked disabled style="pointer-events:none;">
            </a>
        </td>
    </s:if>
									...
									<s:iterator value="bankProducts">
										<input type="hidden" name="product_<s:property value='#purposeCode'/>" value="<s:property value='code'/>" />
									</s:iterator>
