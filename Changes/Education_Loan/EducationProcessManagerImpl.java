...
//          ccmsWriteStatus=true;
          SessionUtil.setConsentSubmitNTBEducation("true");
          if (!ccmsWriteStatus) {
        	  return loanScenarioBean;
          }
      }
...
//            logger.info("OTP verfied for mobileNo ::" + (String.valueOf(appForm.getAppISDCode()) + appForm.getAppMobileNo()).trim() + " with AppSeqId ::" + appSeqId);
  ...
  private boolean writePrivacyConsentToCCMS(ApplicationFormEducationLoan application, ApplicationFormEducationLoanQuote quote,LoanScenarioBean loanScenarioBean,Integer loanTypeId) {
		...
		try {
			...
			quote = educationLoanService.save(quote);
			application = educationLoanService.save(application);
			...
			SMS_TEXT=SMS_TEXT.replaceAll("LOAN_TYPE", "EL");
