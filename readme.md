...
isProductSpecific 
Future scope. Whether consent is 
to be captured against individual 
bank products. 
Yes boolean true, false 
...
String size: 1-50 
bankProducts 
Future scope. If consent is to be 
captured against individual 
products of the bank, the value of 
this field may be used to show as 
selectable items. 
...
The API endpoint provides the latest consent status against the identifier of the data principal. The API 
provides the respective purpose information along with present consent status of the enquired data 
principal. This API may be used by the touchpoints which want to present the consent status to data 
principals. 
This API may be preferably used when the user’s present consent is required to be shown to the user with 
purpose details (may be UI). 
4.3.1. API Details 
HTTP Method POST 
Content Type application/json 
API Request Payload  { 
...
Yes string 1-10 
purposeSetId 
The purpose set Id/Code that the 
integrating channel/touchpoint 
got configured in CCMS. 
Yes string 1-100 
dpCIF 
For bank’s customers, CIF must 
be sent to CCMS for enquiring the 
present consent status to CCMS. 
Use CIF as the primary identifier if 
CIF is available in the bank for the 
data principal. When CIF is 
present in request, dpMobile 
input field will be ignored by 
CCMS. 
